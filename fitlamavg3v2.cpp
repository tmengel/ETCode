...
// main function:
int fitlamavg3v2(){
	std::ofstream avg ("avgLAMResults.dat", std::ofstream::app);
	...
	int stop =110; // breakOut after this many iterations (if achieved); default: 140
	cout << "Flag" << endl;
	while((mikey=(TKey*)next())){
	  breakOutForTesting++;
		if (breakOutForTesting<80) continue;// 140 histograms already analyzed
		...
		gStyle->SetOptFit(1111);// display fit parameters; customizable
		gStyle->SetOptDate();
		// save canvases without displaying them
		c1->Update();
	...
	c1->Update();
		...
		avg <<collEn << "\t"
		    ...
		    << avgNpart <<  "\t"
		    << avgNpart_err <<  "\n";
	string imgPathAndName = "./fittedPlots56/"+histoName+".png";
