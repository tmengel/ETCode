...
TFile* fout = new TFile("poster.root","UPDATE");
TObjArray *betastack2 = new TObjArray(18);	
	...
	int stop =6; // breakOut after this many iterations (if achieved); default: 140
	cout << "Flag" << endl;
	while((mikey=(TKey*)next())){
	  breakOutForTesting++;
		...
		else if(histoName.substr( histoName.length() - 4 ) == "0LAM") collEn = 200;
		...
		*/
		...
		EXPO->SetLineColor(kViolet);
		EXPO->SetLineStyle(3);
		EXPO->SetLineWidth(1);

		EXPO2->SetLineColor(kBlack);
		EXPO2->SetLineStyle(1);
		EXPO2->SetLineWidth(1);
		...
		if(chi2Prob<0.0001) funcBGBW->SetLineColor(kWhite);
if(chi2prob2<0.0001) funcBGBW2->SetLineColor(kWhite);
 if(chi3prob3<0.0001) HAGE->SetLineColor(kWhite);
 if(chi4prob4<0.0001)EXPO->SetLineColor(kWhite);
 if(chi5prob5<5.0001)EXPO2->SetLineColor(kWhite);
	...
	legend->AddEntry(EXPO,"Exponential ","l");
	legend->AddEntry(EXPO2,"Exponential 2","l");
	...
	c1->SetTitle(hisst);
	betastack2->Add(h);
        TGraphErrors *Beta=(TGraphErrors*)betastack2->At(breakOutForTesting);
