...
int fitcompareavg1v2(){
	...
	while((mikey=(TKey*)next())){
	  breakOutForTesting++;
		...
		std::string str1 ("cent5_pi+_Au+Au_7.7");
		...
		if(histoName=="cent8_ka-_Au+Au_7.7"){
		  funcBGBW2->SetParLimits(1,0.0,0.8);//beta
		  funcBGBW2->SetParLimits(2,.01,.2);//temp
		  funcBGBW2->SetParLimits(3,0.01,10);//n

		}
		//cout<<"Froggy frog"<<endl;
		  funcBGBW->SetParLimits(1,0.0,0.99);//beta
		  funcBGBW->SetParLimits(3,0.001,10000);//n
